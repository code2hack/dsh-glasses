...
(function installGlassesSnapshotCore(root) {
  ...
  // -------------------------------------------------------------------------
  function validateWire(snapshot, expectedProtocolMajor, expectedSessionId, expectedServerGeneration) {
    ...
    if (expectedServerGeneration !== undefined && snapshot.serverGeneration !== expectedServerGeneration) return fail('serverGeneration-mismatch', 'serverGeneration ' + snapshot.serverGeneration + ' != expected ' + expectedServerGeneration);
    ...
    if (att.label !== M1_ATTACHMENT_LABEL) return fail('wrong-label', 'attachment label must be "Attached session"');
    ...
    if (caps.liveUpdates !== undefined && caps.liveUpdates !== false) return fail('mutation-capability-enabled', 'capability liveUpdates must be false in M1');
  ...
  // reducer instance, never global state). Returns {ok:true, snapshot:staged}
  // — the only object any later install step may adopt wholesale.
    ...
    return {
      ...
      attachment: {
        ...
        capabilities: (function (c) {
          var o = {};
          for (var ck in c) o[ck] = c[ck];
          return o;
        })(attachment.capabilities),
        agent: (function (a) {
          var o = {};
          for (var ak in a) o[ak] = a[ak];
          return o;
        })(attachment.agent),
        history: {
          ...
          events: history.events.slice(),
        },
      },
      drafts: snapshotArraySlice(raw.drafts),
      conversation: conversation,
      items: items,
    };
  }

  function snapshotArraySlice(arr) {
    var out = [];
  ...
  function stageSnapshot(rawSnapshot, opts) {
    ...
    try {
      var judged = validateWire(rawSnapshot, expectedProtocolMajor, expectedSessionId, expectedServerGeneration);
      if (!judged.ok) return judged;
      return { ok: true, snapshot: buildStaged(rawSnapshot) };
    } catch (e) {
