 ...
 * attachmentId, positive attachment generation, set revision 1, fresh
 * connectionEpoch (caller MUST supply a new value per authenticated bootstrap),
 ...
 * across attachment/agent/history.
 */
export function buildCanonicalSnapshot({ sessionId, projected, agentState, serverGeneration, connectionEpoch, maxEvents }) {
  ...
  // Opaque attachment identity: never the sessionId, never a substring of it.
  const attachmentId = `att:${sg}`;
  if (attachmentId === sid || attachmentId.includes(sid)) {
    throw new SnapshotValidationError("attachmentId-encodes-session", `attachmentId ${attachmentId} encodes sessionId ${sid}`);
  }
