...
function reviewerRules() {
  return [
    "STRICT sequential helper chain: ChatGPT FIRST, fresh native Codex subagent as escalation ONLY, DSH alone only as last resort. ChatGPT and Codex are NEVER requested in parallel for the same planning, progress, debug, or review step. Exact ChatGPT endpoint: mcp-chatgpt ; ChatGPT project = dsh-glasses ; ChatGPT session = CTO.",
    "UNAVAILABLE is recorded only on objective execution failure (request timeout, rate/quota/usage limit, provider/service outage, transport/tool failure that prevents a usable response). A returned technical verdict — UNPASSED, REQUEST_CHANGES, or any blocking finding — is NOT unavailability and must be addressed; never relabel a blocking finding as UNAVAILABLE to bypass it.",
    "Plan before edits: before the first production edit, after local bootstrap/inspection, make ONE bounded attempt to ask ChatGPT for a DETAILED ORDERED implementation+validation plan with a checkable to-do list. If ChatGPT is objectively UNAVAILABLE, ask a fresh native Codex subagent (`subagent_codex`, or the supported equivalent exposed by the pinned DSH deployment) for that plan; ChatGPT unavailability must not deadlock the Ticket. DSH may produce its own plan ONLY if BOTH helpers are unavailable; never make your own plan while a helper can provide one. If a provided plan conflicts with durable authority, report the conflict and request correction; a plan never overrides higher durable authority.",
    "Checkpoints: after EVERY completed to-do item, send a progress checkpoint identifying the ticket, todo-item, status, head, result, validation, evidence, and the next item. Route every checkpoint ChatGPT-first; use fresh Codex for a checkpoint only when ChatGPT is unavailable for it or the item belongs to a chain already escalated after three unsuccessful ChatGPT loops. If both helpers are unavailable, record the checkpoint durably and continue.",
    "One complete ChatGPT loop is: DSH sends one bounded request -> ChatGPT returns guidance or a blocking/non-pass result -> DSH applies it and validates -> the SAME planning/progress/debug/review chain remains unresolved or non-passing. After exactly THREE unsuccessful ChatGPT loops on the same chain, escalate that chain to a fresh Codex subagent instead of starting a fourth ChatGPT loop for that chain; once the escalated chain resolves, ordinary interactions return to ChatGPT-first.",
    "Hard problems: ask ChatGPT FIRST and follow its guidance; implement and validate; if still unresolved continue the same ChatGPT chain; after 3 unsuccessful loops escalate to fresh Codex; if ChatGPT is objectively unavailable escalate to Codex immediately; if Codex is also unavailable continue debugging yourself; you must not wait indefinitely on a helper.",
    "Final review: when acceptance-ready, commit/push the exact candidate FIRST, then ask ChatGPT to review the exact head. If ChatGPT PASSes, the reviewer gate is satisfied — do NOT invoke Codex. If ChatGPT is unavailable, ask a fresh Codex subagent for the exact-head review. If ChatGPT returns a blocking verdict, fix and validate, push the new exact head, and ask ChatGPT again (that is the next loop of the same review chain); after the third unsuccessful ChatGPT review loop, escalate the exact candidate to fresh Codex instead of a fourth ChatGPT request, and continue whatever chain Codex returns. If both helpers are unavailable, you may complete only when every non-review acceptance requirement passes and no unresolved known blocking finding remains (resolve or disprove each with durable evidence).",
    "Codex is a fresh one-shot native DSH subagent, NOT a persistent Ticket worker, and NEVER a parallel peer of ChatGPT. Every invocation runs in this Ticket's workspace (cwd = this worktree), receives ONLY the self-contained bounded git-grounded task (never this DSH conversation history), and returns its final result. Codex planning/progress/debug/review tasks must state: inspect/reason/report only; do not modify the Ticket worktree. There is no persistent Codex session or thread; the dispatcher stores no Codex lifecycle state.",
  ];
}
...
export function bootstrapPrompt({ number, url, milestone, name, baseSha, branch, worktree, project = "dsh-glasses" }) {
  ...
  const context = [
    ...
    "- verify the supported native Codex subagent capability (`subagent_codex`, or the supported equivalent exposed by the pinned DSH deployment) is available for possible escalation (planning/progress/debug/review), or record a genuine provider/tool unavailability;",
  ].join("\n");
  const protocol = [
    "",
    ...
    ...reviewerRules(),
    "",
    "DSH MUST continue until the Ticket completion gate in AGENTS.md / docs/WORKFLOW.md is satisfied. Do not stop merely because a helper timed out, hit a usage limit, or was otherwise unavailable, and do not stop until every known valid blocking finding is resolved or disproved.",
  ].join("\n");
  const closeout = [
    "",
...
export function continuationPrompt({ number, name }) {
  return `Continue Ticket #${number} (DSH session ${name}) toward its completion gate in the SAME session; you are not a replacement session and must not restart from scratch. Re-check the CURRENT durable AGENTS.md and docs/WORKFLOW.md at the current main (workflow authority may have moved since your admission), re-read your active helper-produced plan and your completed/remaining to-do and checkpoint state, then continue the STRICT sequential helper chain: ChatGPT FIRST; fresh native Codex escalation only when ChatGPT is objectively unavailable or the same problem/review chain has survived three unsuccessful ChatGPT loops; you alone only as last resort. Never request ChatGPT and Codex in parallel, apply the final-review routing (ChatGPT PASS -> no Codex), resolve or disprove every known blocking finding, and do not wait indefinitely on any helper. Do not stop until TicketComplete.`;
}
